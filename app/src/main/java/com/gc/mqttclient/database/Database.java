package com.gc.mqttclient.database;
 ...
 * singleton {@link ConnectionHandler} object.
 *
 */
public class Database extends SQLiteOpenHelper implements BaseColumns {
  ...
  //last will
  /** Table column for last will etxt_subscribe_topic **/
  public static final String COLUMN_TOPIC = "etxt_subscribe_topic";
  ...
  /** Table column for the cb_publish_retain state of the message **/
  public static final String COLUMN_RETAINED = "cb_publish_retain";
   ...
   */
  public void persistConnection(Connection connection) throws DatabaseException {
    ...
    if (newRowId == -1) {
      throw new DatabaseException("Failed to persist connection: " + connection.handle());
    }
   ...
   */
  public List<Connection> restoreConnections(Context context) throws DatabaseException
  {
    //columns to return
    ...
    for (int i = 0; i < c.getCount(); i++) {
      if (!c.moveToNext()) { //move to the next item throw persistence exception, if it fails
        throw new DatabaseException("Failed restoring connection - count: " + c.getCount() + "loop iteration: " + i);
      }
      ...
      String etxt_subscribe_topic = c.getString(c.getColumnIndexOrThrow(COLUMN_TOPIC));
      ...
      boolean cb_publish_retain = c.getInt(c.getColumnIndexOrThrow(COLUMN_RETAINED)) == 1 ? true : false;
      ...
      if (etxt_subscribe_topic != null) {
        opts.setWill(etxt_subscribe_topic, message.getBytes(), qos, cb_publish_retain);
      }
