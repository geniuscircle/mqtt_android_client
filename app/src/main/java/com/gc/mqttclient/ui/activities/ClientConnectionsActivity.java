...
import com.gc.mqttclient.listener.ActionListener;
import com.gc.mqttclient.listener.UIElementListener;
import com.gc.mqttclient.handler.MqttCallbackHandler;
import com.gc.mqttclient.helper.MqttTraceCallback;
 ...
 *
 */
public class ClientConnectionsActivity extends ListActivity {
   ...
   */
  private ClientConnectionsActivity clientConnections = this;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    arrayAdapter = new ArrayAdapter<Connection>(this,
            R.layout.listitem_connections);
    setListAdapter(arrayAdapter);

    // get all the available connections
    Map<String, Connection> connections = ConnectionHandler.getInstance(this)
            .getConnections();
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    OnMenuItemClickListener menuItemClickListener = new UIElementListener(this);
  ...
  protected void onResume() {
    ...
    //Recover connections.
    Map<String, Connection> connections = ConnectionHandler.getInstance(this).getConnections();
  ...
  private void connectAction(Bundle data) {
    ...
    client = ConnectionHandler.getInstance(this).createClient(this, uri, clientId);
    ...
    String etxt_subscribe_topic = (String) data.get(ActivityConstants.etxt_subscribe_topic);
    Integer qos = (Integer) data.get(ActivityConstants.qos);
    Boolean cb_publish_retain = (Boolean) data.get(ActivityConstants.cb_publish_retain);
    ...
    final ActionListener callback = new ActionListener(this,
            Action.CONNECT, clientHandle, actionArgs);

    boolean doConnect = true;

    if ((!message.equals(ActivityConstants.empty))
            || (!etxt_subscribe_topic.equals(ActivityConstants.empty))) {
      // need to make a message since last will is set
      try {
        conOpt.setWill(etxt_subscribe_topic, message.getBytes(), qos.intValue(),
                cb_publish_retain.booleanValue());
      }
      catch (Exception e) {
    ...
    ConnectionHandler.getInstance(this).addConnection(connection);
    if (doConnect) {
      try {
  ...
  private class LongClickItemListener implements OnItemLongClickListener, ActionMode.Callback, OnClickListener {
    ...
    public boolean onCreateActionMode(ActionMode mode, Menu menu) {
      ...
      inflater.inflate(R.menu.menu_activity_client_connections_contextual, menu);
    ...
    private void delete()
    {
      ...
      if (connection.isConnectedOrConnecting()) {
        ...
        ConnectionHandler.getInstance(clientConnections).removeConnection(connection);
      }
